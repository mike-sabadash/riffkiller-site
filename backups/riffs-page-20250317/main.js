...
// Инициализация (риффы из API /api/riffs/list.php при наличии, иначе из riffsDatabase)
document.addEventListener('DOMContentLoaded', () => {
    ...
    fetch('/api/riffs/list.php').then(r => r.json()).then(list => {
...
function createRiffCard(riff) {
    ...
    const difficultyBadge = `<div class="riff-difficulty" style="position:absolute; top:16px; left:16px; background-color:${colors[riff.difficulty]}; padding:6px 12px; border-radius:12px; font-size:12px; color:white; z-index:2;">${labels[riff.difficulty]}</div>`;
    const freeTag = riff.isFree ? '<div class="riff-free-tag">FREE</div>' : '';

    card.innerHTML = `
        <div class="riff-image-container" style="position:relative; width:100%; height:100%;">
            <!-- Прелоадер - ЯРКАЯ ВЕРСИЯ -->
            ...
            <img src="${riff.thumbnail}" alt="${riff.song}" class="riff-image" style="opacity:0; transition:opacity 0.3s ease; width:100%; height:100%; object-fit:cover; position:absolute; top:0; left:0;" 
                 onload="this.style.opacity='1'; this.parentElement.querySelector('.riff-image-preloader').style.display='none';"
                 onerror="this.src='https://via.placeholder.com/214x180/2a2d42/ffffff?text=${encodeURIComponent(riff.song)}'; this.style.opacity='1'; this.parentElement.querySelector('.riff-image-preloader').style.display='none';">
        </div>
        <div class="riff-overlay"></div>
        ${difficultyBadge}
        <button class="riff-favorite-btn ${riff.isFavorite ? 'active' : ''}" 
                aria-label="${riff.isFavorite ? 'Remove from favorites' : 'Add to favorites'}">
            ${favoriteIcon}
        </button>
        <div class="riff-content">
            <div class="riff-info">
                <div class="riff-song">${riff.song}</div>
    ...
    const favBtn = card.querySelector('.riff-favorite-btn');
    favBtn.addEventListener('click', (e) => {
        e.stopPropagation();
...
card.addEventListener('click', () => {
    // Free riff: always open practice, no trial/subscription checks
    if (riff.isFree) {
    ...
    const hasSubscription = window.patreonAuth && typeof window.patreonAuth.hasActiveSubscription === 'function' && window.patreonAuth.hasActiveSubscription();

    // Без подписки открывается только free-рифф; платные — только с подпиской
    if (hasSubscription) {
        appState.currentRiffId = riff.id;
        saveAppState();
        document.body.style.opacity = '0.7';
...
function initNavigationArrows() {
    ...
    const updateArrows = () => {
        ...
        next.disabled = scrollLeft + clientWidth >= scrollWidth - 10;
